...
describe('StateAsync Retry Logic', () => {
  ...
  describe('withRetry()', () => {
    ...
    test('should throw last error when all retries exhausted', async () => {
      ...
      await expect(stateAsync.withRetry(promiseFactory, 2, 1)).rejects.toThrow('Always fails')
  ...
  describe('setStateAsync with retry integration', () => {
    test('should properly retry async state operations', async () => {
      ...
      expect(mockCallbacks.onSetState).toHaveBeenCalledWith(
        'test.path',
        'final data',
        expect.any(Object)
      )
  ...
  describe('Edge cases and error handling', () => {
    test('should handle zero retries', async () => {
      ...
      await expect(stateAsync.withRetry(promiseFactory, 0, 10)).rejects.toThrow('Immediate failure')
