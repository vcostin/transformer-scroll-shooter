...
import { pathOr, safeResolveReference } from '@test/systems/StateUtils.js'
...
describe('Enhanced StateUtils', () => {
  ...
  describe('safeResolveReference()', () => {
    ...
    test('should handle numeric string references', () => {
      ...
      const result = safeResolveReference('$config.limit', 'validation.path', stateWithNumbers, 0)
    ...
    test('should return fallback for non-numeric strings when expecting numbers', () => {
      const result = safeResolveReference('$user.profile.name', 'validation.path', testState, 0)
  ...
  describe('Integration with validation scenarios', () => {
    test('should safely validate number ranges', () => {
      ...
      // Test min validation
      const minValue = safeResolveReference(
      ...
      // Test max validation
      const maxValue = safeResolveReference(
