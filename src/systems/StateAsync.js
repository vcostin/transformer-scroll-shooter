...
export class StateAsync {
    ...
    async setStateAsync(path, valueOrPromise, options = {}) {
        ...
        try {
            ...
            if (this.options.enableDebug) {
                console.log(`✅ StateAsync: setStateAsync('${path}') completed in ${operationTime.toFixed(2)}ms`);
            }
            ...
            if (this.options.enableDebug) {
                console.error(`❌ StateAsync: setStateAsync('${path}') failed after ${operationTime.toFixed(2)}ms:`, error);
            }
        ...
        for (let attempt = 0; attempt <= retryAttempts; attempt++) {
            try {
                ...
                if (attempt < retryAttempts) {
                    ...
                    if (this.options.enableDebug) {
                        console.log(`🔄 StateAsync: Retrying operation (attempt ${attempt + 2}/${retryAttempts + 1})`);
                    }
                }
            }
        }
    ...
    async executeSequentially(operations) {
        ...
        for (const operation of operations) {
            const result = await this.setStateAsync(operation.path, operation.valueOrPromise, operation.options);
            results.push(result);
        }
        return results;
    }
    ...
    updateAverageTime(time) {
        ...
        if (totalOps === 1) {
            ...
            this.stats.averageOperationTime = (
                this.stats.averageOperationTime * (totalOps - 1) + time
            ) / totalOps;
        }
    }
