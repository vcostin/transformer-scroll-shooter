    ...
    validateIntegrity() {
        ...
        for (const [subscriptionId, info] of this.subscriptionIndex) {
            ...
            if (!subscriptions) {
                ...
                issues.push(`Indexed subscription ${subscriptionId} has invalid index ${index} for path: ${path}`);
        ...
        for (const [path, subscriptions] of this.subscriptions) {
            for (let i = 0; i < subscriptions.length; i++) {
                ...
                if (!indexInfo) {
                    ...
                    issues.push(`Subscription ${subscription.id} index mismatch: expected ${path}[${i}], got ${indexInfo.path}[${indexInfo.index}]`);
                }
            }
        }
