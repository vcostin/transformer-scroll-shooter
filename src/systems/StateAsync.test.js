...
describe('StateAsync', () => {
  ...
  describe('Loading state management', () => {
    it('should set and clear loading state for successful operation', async () => {
      ...
      expect(mockSetState).toHaveBeenNthCalledWith(3, 'data.value', 'final value', {
        loadingPath: 'data.loading'
      })
    })
  ...
  describe('Error state management', () => {
    it('should set error state on promise rejection', async () => {
      ...
      expect(mockSetState).toHaveBeenCalledWith('data.error', 'Test error message', {
        skipHistory: true
      })
    })
    ...
    it('should handle unknown error types', async () => {
      ...
      expect(mockSetState).toHaveBeenCalledWith('data.error', 'Unknown error', {
        skipHistory: true
      })
    })
  })
  ...
  describe('Retry functionality', () => {
    ...
    it('should fail after exhausting retry attempts', async () => {
      ...
      await expect(
        stateAsync.setStateAsync('test.path', alwaysFailPromise, {
          retryAttempts: 2,
          retryDelay: 10
        })
  ...
  describe('Batch operations', () => {
    ...
    it('should handle batch errors', async () => {
      ...
      await expect(
        stateAsync.batchSetStateAsync(operations, {
          errorPath: 'global.error'
        })
      ).rejects.toThrow('Batch error')

      expect(mockSetState).toHaveBeenCalledWith('global.error', 'Batch error', {
        skipHistory: true
      })
    })

    it('should emit batch events', async () => {
      const operations = [{ path: 'data.a', valueOrPromise: Promise.resolve('A'), options: {} }]

      await stateAsync.batchSetStateAsync(operations)

      expect(mockEmitEvent).toHaveBeenCalledWith(
        'state:async-batch-success',
        expect.objectContaining({
          operations: ['data.a'],
          results: [true]
        })
  ...
  describe('Edge cases', () => {
    ...
    it('should handle promise rejection with no error message', async () => {
      ...
      expect(mockSetState).toHaveBeenCalledWith('test.error', 'Unknown error', {
        skipHistory: true
      })
    })
  })
