...
export class CompatibilityAnalyzer {
  ...
  removeUnnecessaryChecks(content) {
    ...
    content = content.replace(/if\s*\(\s*this\.eventDispatcher\s*\)\s*{([^}]*)}/gs, '$1')

    content = content.replace(/if\s*\(\s*this\.stateManager\s*\)\s*{([^}]*)}/gs, '$1')
    ...
    const analyzeDirectory = dir => {
      ...
      items.forEach(item => {
        ...
        if (stat.isDirectory() && !item.startsWith('.') && item !== 'node_modules') {
          ...
          const totalIssues =
            analysis.legacyComments.length +
      ...
      summary: {
        ...
        averageReduction:
          this.removals.length > 0
            ? this.removals.reduce((sum, r) => sum + r.sizeReduction, 0) / this.removals.length
            : 0
      }
    }
